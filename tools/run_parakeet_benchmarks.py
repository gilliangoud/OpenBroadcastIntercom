#!/usr/bin/env python3
"""Run a RedLine benchmark corpus through NVIDIA NeMo ASR candidates."""
...
DEFAULT_MODELS: list[tuple[str, str]] = [
    ("parakeet-unified-0.6b", "nvidia/parakeet-unified-en-0.6b"),
    ("nemotron-streaming-0.6b", "nvidia/nemotron-speech-streaming-en-0.6b"),
...
def enrich_score(
    ...
    *,
    ...
    result["model"] = {"id": model_id, "runtime": "nemo-asr", "repo": repo}
    result["adapter"] = {
...
def run_model(
    *,
    ...
    command = [
        ...
        str(args.batch_size),
        "--left-context-secs",
        str(args.left_context_secs),
        "--chunk-secs",
        str(args.chunk_secs),
        "--right-context-secs",
        str(args.right_context_secs),
    ]
    if args.timestamps:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--python",
        type=Path,
        default=ROOT / "artifacts/transcription-benchmarks/.venv-nemo/bin/python",
        help="Python executable with NVIDIA NeMo ASR installed",
    )
    parser.add_argument(
        "--hf-home",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    for model_id, repo in model_specs:
        ...
        try:
            ...
            error_result = {
                "model": {"id": model_id, "runtime": "nemo-asr", "repo": repo},
                "error": str(exc),
            }
