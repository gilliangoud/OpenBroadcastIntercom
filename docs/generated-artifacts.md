...
Downloaded Whisper `.bin`/`.gguf` files and generated Core ML model packages
are local assets described by `intercom-models/manifest.json`; do not commit
them. Large approved non-Whisper model files are source only when they are part
of the curated Git LFS set documented in `THIRD_PARTY_NOTICES.md`. Do not
commit unreviewed model weights or model archives.
